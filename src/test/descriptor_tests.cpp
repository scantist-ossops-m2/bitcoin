...
constexpr int SIGNABLE_FAILS = 64; // We can sign with this descriptor, but actually trying to sign will fail
        ...
        for (int t = 0; t < 2; ++t) {
            ...
            for (size_t n = 0; n < spks.size(); ++n) {
                ...
                if (flags & (SIGNABLE | SIGNABLE_FAILS)) {
                    ...
                    sig_data.hash_preimages = preimages;
                    const bool signing_res = SignSignature(Merge(keys_priv, script_provider), spks[n], spend, 0, 1, SIGHASH_ALL, sig_data);
                    BOOST_CHECK_MESSAGE(signing_res == !(flags & SIGNABLE_FAILS), prv);
                }
