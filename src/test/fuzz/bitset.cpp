...
void TestType(Span<const uint8_t> buffer)
{
    ...
    LIMITED_WHILE(buffer.size() > 0, 1000) {
        unsigned command = ReadByte(buffer) % 64;
        ...
        while (true) {
            if (dest < sim.size() && command-- == 0) {
                ...
                unsigned r1 = rng() % S::Size(), r2 = rng() % S::Size(), r3 = rng() % S::Size();
                compare_fn(dest);
                sim[dest].reset();
                real[dest] = {r1, r2, r3};
                sim[dest].set(r1);
                ...
                unsigned r1 = rng() % S::Size(), r2 = rng() % S::Size();
                sim.emplace_back();
                sim.back().set(r1);
                sim.back().set(r2);
                ...
                real[dest] ^= real[src];
                break;
            } else if (src < sim.size() && dest < sim.size() && sim[src].any() && command-- == 0) {
                /* JustFirst */
                compare_fn(dest);
                unsigned first = 0;
                while (!sim[src][first]) ++first;
                sim[dest].reset();
                sim[dest][first] = true;
                real[dest] = real[src].JustFirst();
                break;
