    ...
    CScript ToScript(const Ctx& ctx) const
    {
        ...
        auto upfn = [&ctx](bool verify, const Node& node, Span<CScript> subs) -> CScript {
            switch (node.nodetype) {
                ...
                case NodeType::WRAP_A: return CatMany((CScript() << OP_TOALTSTACK), std::move(subs[0]), (CScript() << OP_FROMALTSTACK));
                case NodeType::WRAP_S: return CatMany((CScript() << OP_SWAP), std::move(subs[0]));
                case NodeType::WRAP_C: return CatMany(std::move(subs[0]), CScript() << (verify ? OP_CHECKSIGVERIFY : OP_CHECKSIG));
                case NodeType::WRAP_D: return CatMany((CScript() << OP_DUP << OP_IF), std::move(subs[0]), (CScript() << OP_ENDIF));
                case NodeType::WRAP_V: return CatMany(std::move(subs[0]), (node.subs[0]->GetType() << "x"_mst ? (CScript() << OP_VERIFY) : CScript()));
                case NodeType::WRAP_J: return CatMany((CScript() << OP_SIZE << OP_0NOTEQUAL << OP_IF), std::move(subs[0]), (CScript() << OP_ENDIF));
                case NodeType::WRAP_N: return CatMany(std::move(subs[0]), CScript() << OP_0NOTEQUAL);
                case NodeType::JUST_1: return CScript() << OP_1;
                case NodeType::JUST_0: return CScript() << OP_0;
                case NodeType::AND_V: return CatMany(std::move(subs[0]), std::move(subs[1]));
                case NodeType::AND_B: return CatMany(std::move(subs[0]), std::move(subs[1]), (CScript() << OP_BOOLAND));
                case NodeType::OR_B: return CatMany(std::move(subs[0]), std::move(subs[1]), (CScript() << OP_BOOLOR));
                case NodeType::OR_D: return CatMany(std::move(subs[0]), (CScript() << OP_IFDUP << OP_NOTIF), std::move(subs[1]), (CScript() << OP_ENDIF));
                case NodeType::OR_C: return CatMany(std::move(subs[0]), (CScript() << OP_NOTIF), std::move(subs[1]), (CScript() << OP_ENDIF));
                case NodeType::OR_I: return CatMany((CScript() << OP_IF), std::move(subs[0]), (CScript() << OP_ELSE), std::move(subs[1]), (CScript() << OP_ENDIF));
                case NodeType::ANDOR: return CatMany(std::move(subs[0]), (CScript() << OP_NOTIF), std::move(subs[2]), (CScript() << OP_ELSE), std::move(subs[1]), (CScript() << OP_ENDIF));
                case NodeType::MULTI: {
                ...
                case NodeType::THRESH: {
                    ...
                    for (size_t i = 1; i < subs.size(); ++i) {
                        script = Cat(std::move(script), std::move(subs[i])) << OP_ADD;
                    }
