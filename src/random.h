...
/** Object representing a deterministic set of events generated by Poisson process. */
class PoissonProcessRandom
{
    uint64_t m_k0, m_k1;
    std::chrono::microseconds m_interval;

public:
    PoissonProcessRandom() = delete;
    PoissonProcessRandom(const PoissonProcessRandom&) = default;
    PoissonProcessRandom& operator=(const PoissonProcessRandom&) = default;

    /** Construct a random PoissonProcessRandom object with the specified average interval. */
    PoissonProcessRandom(std::chrono::seconds average_interval);
    /** Construct a deterministic PoissonProcessRandom object. */
    PoissonProcessRandom(std::chrono::seconds average_interval, uint64_t k0, uint64_t k1);

    /** Find the first event (strictly) after now. */
    std::chrono::microseconds Next(std::chrono::microseconds now);
};
