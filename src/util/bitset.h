...
public:
    ...
    /** Construct a set with just the first element of this (requires Any()).  */
    constexpr IntBitSet JustFirst() const noexcept { return I(m_val & -m_val); }
...
public:
    ...
    /** Construct a set with just the first element of this (requires Any()). */
    constexpr MultiIntBitSet JustFirst() const noexcept
    {
        MultiIntBitSet ret;
        unsigned p = 0;
        while (m_val[p] == 0) ++p;
        ret.m_val[p] = m_val[p] & -m_val[p];
        return ret;
    }
    /** Find the last element (requires Any()). */
