...
{
    int step = 1;
    std::vector<std::pair<arith_uint256, uint256>> have;
    if (index == nullptr) return have;

    have.reserve(32);
    while (index) {
        have.emplace_back(index->nChainWork, index->GetBlockHash());
...
CBlockLocator BuildLocator(std::vector<std::pair<arith_uint256, uint256>>&& entries)
{
    ...
    // Reverse sort: highest work first.
    std::stable_sort(entries.begin(), entries.end(), [](const auto& a, const auto& b) { return a.first > b.first; });
    for (const auto& [_, hash] : entries) {
        // Build the locator using the hashes in entries, sorted by highest work first, skipping duplicates.
        if (hashes.empty() || hash != hashes.back()) hashes.push_back(hash);
    }
